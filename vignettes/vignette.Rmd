...
pacman::p_load(VLSM, dplyr, ggplot2, rgrass7, raster, link2GI, sf)


# LOAD THE DATA OF THE PACKAGE:
data(VLSM_data, package = "VLSM")

# ... CORINE Land Cover (CLC) Germany from the year 2012
# ... Jena and Saalze-Holzand
...
ggplot2::ggplot() + 
  ...
  ggplot2::geom_sf_text(data = ADMIN_DIS, aes(label = GEN), colour = "white", size = 3.6) + 
  ggplot2::theme_bw() + 
...
knitr::kable(data.frame(Name = ADMIN_DIS$GEN,
           CLC2012_CUT = VLSM_data_result$TH12_mesh$mesh$mEff_CUT,
           CLC2018_CUT = VLSM_data_result$TH18_mesh$mesh$mEff_CUT,
           CLC2012_CBC2 = VLSM_data_result$TH12_mesh$mesh$mEff_CBC2,
           CLC2018_CBC2 = VLSM_data_result$TH18_mesh$mesh$mEff_CBC2))
```

Interesting, depending on the measurement the results show a contrary trend: While the cutting-out (CUT) procedure shows an increasing landscape fragmentation from 2012 to 2018, the cross-boundary connections (CBC) procedure shows a decreasing landscape fragmentation. So take care and choose the appropriate result for your study design!
...
knitr::kable(data.frame(Name = ADMIN_DIS$GEN,
           CLC2012_FFE = VLSM_data_result$TH12_urbanSprawl$FFE$FFE,
           CLC2018_FFE = VLSM_data_result$TH18_urbanSprawl$FFE$FFE))
```
...
Take care: The result varies depending on the transformation-function and on the mesh sizes. With a higher mesh size, it is possible to miss small human settlements resulting in a lower value of *urban sprawl*.
...
knitr::kable(data.frame(Name = ADMIN_DIS$GEN,
           R = VLSM_data_result$TH18_integration$integration_index$R))
```
...
```{r, echo = FALSE, results = 'MSI', eval = FALSE}
knitr::kable(data.frame(Name = ADMIN_DIS$GEN,
           CLC2012_MSI = VLSM_data_result$CLC2012_MSI$MSI$MSI,
           CLC2012_AWMSI = VLSM_data_result$CLC2012_MSI$MSI$AWMSI,
           CLC2018_MSI = VLSM_data_result$CLC2018_MSI$MSI$MSI,
           CLC2018_AWMSI = VLSM_data_result$CLC2018_MSI$MSI$AWMSI))
```
...
knitr::kable(data.frame(Name = ADMIN_DIS$GEN,
           CLC2012_EdgeLength = VLSM_data_result$TH12_EdgeLength$edge_length$L,
           CLC2018_EdgeLength = VLSM_data_result$TH18_EdgeLength$edge_length$L))
```
...
knitr::kable(data.frame(Name = ADMIN_DIS$GEN,
           CLC2012_Shannon = VLSM_data_result$TH12_Shannon$SHDI$SHDI,
           CLC2018_Shannon = VLSM_data_result$TH18_Shannon$SHDI$SHDI))
```


The district of Jena has a higher *Shannon Diversity Index* (1.1) than Saale-Holzland (0.9), and the index is only slightly changing from 2012 to 2018. The higher diversity for Jena is quite surprising, and appears even contradictory in an ecological sense.
